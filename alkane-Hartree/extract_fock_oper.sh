...
outfile=/cluster/home/stig/benchmarks-mrchem/ref/Hartree/fock_oper.csv
echo "molecule,prec,MPI,OMP,Nuclear pot,Coulomb dens,Coulomb pot,Total Fock" > ${outfile}
        ...
        for mpi in 008 016 032 048 064 080 096 112 128 144 160; do
            for omp in 08 16 32; do
                ...
                if [ -f ${inpfile} ]; then
                    ...
                    echo "alkane_${mol},${prec},${mpi},${omp},${nuc_pot},${coul_dens},${coul_pot},${tot_fock}" >> ${outfile}
                fi
            done
        done
    done
