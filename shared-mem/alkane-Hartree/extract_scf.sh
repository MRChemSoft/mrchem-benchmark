...
outfile=/cluster/home/stig/benchmarks-mrchem/shared-mem/Hartree/scf.csv
            ...
            for omp in 08 16 32; do
                ...
                if [ -f ${inpfile} ]; then
                    ...
                    scf_tot=`get_scf_cycle 1 ${inpfile} \
                        | grep -m 1 '### Wall time' \
                        | awk '{ print $4 }'`
                    echo "ch4_${mol},${mpi},${omp},${localize},${scf_energy},${helmholtz},${lowdin},${kain},${fock_setup},${fock_calc},${scf_tot}" >> ${outfile}
                fi
            done
        done
    done
