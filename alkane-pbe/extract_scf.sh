...
outfile=/cluster/home/stig/benchmarks-mrchem/shared-mem/DFT/scf.csv
echo "molecule,prec,MPI,OMP,Localize,SCF energy,Helmholtz,Lowdin,KAIN,Fock operator,Fock matrix,Total SCF" > ${outfile}
            ...
            for omp in 08 16 32; do
                ...
                if [ -f ${inpfile} ]; then
                    ...
                    scf_tot=`get_scf_cycle 1 ${inpfile} \
                        | grep -m 1 '### Wall time' \
                        | awk '{ print $4 }'`
                    echo "ch4_${mol},${prec},${mpi},${omp},${localize},${scf_energy},${helmholtz},${lowdin},${kain},${fock_setup},${fock_calc},${scf_tot}" >> ${outfile}
                fi
            done
        done
    done
